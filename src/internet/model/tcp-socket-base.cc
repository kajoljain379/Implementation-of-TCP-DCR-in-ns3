  ...
  if (m_tcb->m_congState == TcpSocketState::CA_DISORDER)
    {
      ...
      // go to step (4).
      else if (!m_dcrEnabled)
        {
          m_txBuffer->SetDupAckThresh(m_retxThresh);
          if(m_txBuffer->IsLost (m_highRxAckMark + m_tcb->m_segmentSize))
          {
            EnterRecovery ();
            NS_ASSERT (m_tcb->m_congState == TcpSocketState::CA_RECOVERY);
          } 
      ...
      // go to step (4).
        ...
        {
          m_txBuffer->SetDupAckThresh(m_dcrRetxThresh);
   //       m_txBuffer->Update (list);
          if(m_txBuffer->IsLost (m_highRxAckMark + m_tcb->m_segmentSize))
          {
          EnterRecovery ();
          NS_ASSERT (m_tcb->m_congState == TcpSocketState::CA_RECOVERY);
